...
Name "Rainmeter"
VIAddVersionKey "CompanyName" "Rainmeter x DroptopFour"
VIAddVersionKey "ProductName" "Rainmeter x DroptopFour"
VIAddVersionKey "FileDescription" "Rainmeter x Droptop Installer"
VIAddVersionKey "FileVersion" "${VERSION_FULL}"
VIAddVersionKey "ProductVersion" "${VERSION_FULL}"
VIAddVersionKey "OriginalFilename" "${OUTFILE}"
VIAddVersionKey "LegalCopyright" "© ${BUILD_YEAR} Rainmeter Team x DroptopFour Team"
...
!macro CopyOtherPlugins DIR ARCH
	ClearErrors
	CreateDirectory "$APPDATA\Rainmeter\Plugins"
	
	${If} $InstallPortable <> 1
		SetOutPath "$APPDATA\Rainmeter\Plugins"
	${Else}
		SetOutPath "$INSTDIR\Plugins"
	${EndIf}

	File /x *Example*.dll "..\OtherPlugins\${DIR}\*.dll"
!macroend
...
!ifdef INCLUDEFILES
	...
	${If} $instArc == "x86"
		...
		!insertmacro CopyOtherPlugins "x32" "x86"
	${Else}
		!insertmacro InstallFiles "x64" "x64"
		!insertmacro CopyOtherPlugins "x64" "x64"
	${EndIf}
...
!endif
	...
	${Else}
		${IfNot} ${FileExists} "Rainmeter.ini"
			CopyFiles /SILENT "$INSTDIR\Defaults\Layouts\droptop default\Rainmeter.ini" "$INSTDIR\Rainmeter.ini"
		${EndIf}
